...
//        Symbols symbols = restTemplate.getForObject(URL + request.getStock(), Symbols.class);
//        Symbol symbol = symbols.getSymbolList().get(0);

        TransactionMethodService transactionMethodService = new TransactionMethodService();
        ...
        mstmb.setStock(symbol.getId());
        mstmb.setStockName(symbol.getShortname());
        mstmb.setMarketType(symbol.getMtype());
        mstmb.setCurPrice(symbol.getDealprice());
...
//====================================================================================================
    ...
    //查詢即時價API介接(一次可查多張)
    @Cacheable(cacheNames = "MstnbStock-cache", key = "#request.getStock()")
    public ApiResponse postMstmbByStock2(MstmbRequest request) {
        ...
        //      等於下面一行
        TransactionMethodService transactionMethodService = new TransactionMethodService();
        List<Symbol> symbols = transactionMethodService.api(request.getStock());

        apiResponse.setStatus("查詢成功");
        apiResponse.setSymbolList(symbols);
...
//    @Cacheable(cacheNames = "MstnbStock-cache", key = "#stock")
//    public ApiResponse postMstmbByStock2(String stock) {
//
//        ApiResponse apiResponse = new ApiResponse();
//
//        //      @Autowired
