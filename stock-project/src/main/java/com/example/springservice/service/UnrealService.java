...
//            return new UnrealDetailResponse(null,"002","分行代碼輸入錯誤");
            ...
            List<UnrealDetailResult> resultList = transactionMethodService.getUnrealDetail(request.getStock(), request.getBranchNo(), request.getCustSeq(), request);
            if (resultList.isEmpty()) { //  避免篩選完出現沒有資料的情況
                unrealDetailResponse.setResponseCode("001");
                unrealDetailResponse.setMessage("查無符合資料");
    ...
    public UnrealSumResponse findUnrealSum(UnrealRequest request) {
        ...
        if (isBlank(request.getBranchNo()) || request.getBranchNo().length() != 4) {
            ...
            for (String stock : stockList) {    //  用使用者所持股票去跑迴圈
                ...
                //  將同檔股票的未實現損益明細放入同一陣列
                List<UnrealDetailResult> detailList = transactionMethodService.getUnrealDetail(stock, request.getBranchNo(), request.getCustSeq(), request);
                ...
                unrealSunResult.setStockName(mstmbRepository.findByStock(stock).getStockName());
                unrealSunResult.setNowPrice(mstmbRepository.findCurPriceByStock(stock));    //  == mstmbRepository.findByStock(stock).getCurPrice()
...
//        System.out.println(1 / 0);  //  測試rollback(@Transactional)-->如果有此行，兩個表都不會新增;沒加@Transactional，則會新增hcmio，但不會新增tcnud(responseBody都顯示 005伺服器忙碌中，請稍後嘗試)
        ...
        if (null != tcnudRepository.findByBranchNoAndCustSeq(request.getBranchNo(), request.getCustSeq()).toString()) {
            ...
            unrealDetailResult.setStockName(mstmb.getStockName()); //   明細檔抓股票名稱
            unrealDetailResult.setBuyPrice(tcnud.getPrice());
            unrealDetailResult.setNowPrice(mstmb.getCurPrice());    //  明細檔抓股票現值
            ...
            unrealDetailResult.setMarketValue(calculateService.getMarketValue(unrealDetailResult.getRemainQty(), mstmb.getCurPrice()));
